...
public class UI {
	...
	public void addTerCard(Player player) {
		player.addCard(card);
	}

	public boolean checkNumCards(Player player) {
	...
	public int numOfReinforcements(int count) {
		int start = 4;
		return switch (count) {
			case 1 -> start;
			case 2, 3, 4 -> start += 2;
	...
	public void exchange(Player player) {
		...
		count++;
		message = makeLongName(player) + ": EXCHANGE CARDS: Enter insignias of cards you wish to exchange";
		displayString(message);
		response = commandPanel.getCommand();
		displayString(PROMPT + response);
		int reinforcements = numOfReinforcements(count);
		//HERE
